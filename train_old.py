...
#@click.option('--lr', default=1e-4, help='Learning rate') # HARDCODED LRs INSTEAD
...
@click.option('--end_iter', default=27335, help='Ending iteration') # 50k images / 64 batch size = 781. total iters = 781 * 35 epochs = 27335
...
def train(train_dir, validation_dir, image_data_type, output_path, dim, critic_iters, gen_iters, batch_size, noisy_label_prob, start_iter, end_iter, gp_lambda, num_workers, saving_step, training_class, val_class, restore_mode):
    ...
    for iteration in range(start_iter, end_iter):
        ...
        for i in range(gen_iters):
            ...
            aG.zero_grad()
            try:
                noisy_img, clean_img, *_ = next(dataiter)
